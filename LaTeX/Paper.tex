...
These restrictions are implemented as follows: We divided the text $T$ into separate lines $L=\{W,H\}$ which are in turn made up of words $W$ and whitespace $H$. Each word $w \in W$ is surrounded by a bounding rectangle $r$, %Definition schreiben?
which marks the space leaders aren't allowed to cross. For a graphical representation see Fig.~\ref{fig:wbound}.
...
For each whitespace character $w \in W$, one vertex is placed above and below, so that it has the maximum distance from both of its adjacent lines. The start and end of each line also recieve a pair of vertices each. Sites will be represented by the insertion of additional vertices at appropriate places. The edges between the vertices are all perfectly horizontal or vertical, and do neither intersect with any bounding rectangle, nor any vertices other than their starting and ending vertex.The resulting graph looks similar to Fig.~\ref{fig:rgraph}.
