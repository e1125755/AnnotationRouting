...
%Add Test data generation/retrieval here?
...
Annotation sites are inserted into the text on either a normal or uniform distribution, and their labels contain similarly generated texts of randomized length.

To enable the inspection of individual texts, each text was generated from a randomized seed that is saved at time of creation. 

\section{Testing methods}

All tests were run under 64-bit Windows 7 on a dual-core laptop\footnote{Dell Latitude E6400, Intel Core2 Duo, 4047 MB Virtual Memory on a SSD}.
...
Unsurprisingly, the amount of labeling area space used by annotations increases with the total amount of annotations in the text, but the ratio of successful routings to total sites goes down. 
%While increasing the standard deviation for the annotation placement process also allowed some texts to reach a better label area usage, the average results became worse for that category, whereas the percentage of successful routings moves closer to the 50\% mark.
